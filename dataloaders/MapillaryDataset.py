...
class MSLS(Dataset):
    def __init__(self, input_transform = None):
        ...
        self.dbImages = np.load('datasets/xinzu_val/xinzu_val_dbImages.npy')
        
        # hard coded query image names.
        self.qImages = np.load('datasets/xinzu_val/xinzu_val_qImages.npy')
        
        # hard coded index of query images
        self.qIdx = np.load('datasets/xinzu_val/xinzu_val_qIdx.npy')
        
        # hard coded groundtruth (correspondence between each query and its matches)
        self.pIdx = np.load('datasets/xinzu_val/xinzu_val_pIdx.npy', allow_pickle=True)
...
if __name__ == '__main__':
    ...
    print(a)
    print(dataset.pIdx)
    print(f'Ten of dbImages:{len(dataset.dbImages)}')
