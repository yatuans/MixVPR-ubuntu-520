...
from dataloaders.SatelliteTileAreasDataset import GSVCitiesDataset
# from . import PittsburgDataset
from . import XinzuDataset
from . import UkrianPartDataset
    ...
    def setup(self, stage):
        if stage == 'fit':
            ...
            for valid_set_name in self.val_set_names:
                ...
                #         input_transform=self.valid_transform))
                # el
                if valid_set_name.lower() == 'xinzu_val':
                    self.val_datasets.append(XinzuDataset.MSLS(
                        input_transform=self.valid_transform))
                elif valid_set_name.lower() == 'ukrian-part_val':
                    self.val_datasets.append(UkrianPartDataset.MSLS(
                        input_transform=self.valid_transform))
                else:
                    print(
