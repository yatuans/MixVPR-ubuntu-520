...
class MSLS(Dataset):
    def __init__(self, input_transform = None):
        ...
        self.dbImages = np.load('datasets/xinzu_val/xinzu_dbImages.npy')
        
        # hard coded query image names.
        self.qImages = np.load('datasets/xinzu_val/xinzu_qImages.npy')
        
        # hard coded index of query images
        self.qIdx = np.load('datasets/xinzu_val/xinzu_qIdx.npy')
        
        # hard coded groundtruth (correspondence between each query and its matches)
        self.pIdx = np.load('datasets/xinzu_val/xinzu_pIdx.npy', allow_pickle=True)
